...
namespace Shapes {
	...
	[ExecuteAlways] public class SpinningColorDiscs : ImmediateModeShapeDrawer {
		...
		public override void DrawShapes( Camera cam ) {
			...
			using( Draw.Command( cam ) ) { // all immediate mode drawing should happen within these using-statements
				...
				for( int i = 0; i < discCount; i++ ) {
					...
					Color color = Color.HSVToRGB( t, 0.5f, 1 );
					Vector2 pos = GetDiscPosition( t );
					float scale = GetDiscScale(t);
					Draw.Disc( pos, GetDiscScale(t, Mathf.Sin(Time.time)) * discRadius, color ); // This is the actual Shapes draw command
				}
			}
		}
		...
		float GetDiscScale(float t, float phaseRads = 0)
        {
			float scale = Mathf.Abs(Mathf.Sin(GetDiscPosition(t).y / Vector3.Magnitude(GetDiscPosition(t))) + phaseRads);
			return scale;
        }
